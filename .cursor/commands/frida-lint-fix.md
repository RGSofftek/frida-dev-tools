...
1. Run: `python resources/cli-tools/frida_lint.py format <target>`
2. Run: `python resources/cli-tools/frida_lint.py check --json <target>` and capture full stdout and the exit code.
...
1. After the user approves (all or a subset):
   - For approved diagnostics that are fixable by the linter: you may run `python resources/cli-tools/frida_lint.py check --fix <target>` (and `--unsafe-fixes` only if the user explicitly allowed unsafe fixes) to apply those fixes in one go.
...
2. Run again: `python resources/cli-tools/frida_lint.py format <target>` then `python resources/cli-tools/frida_lint.py check --json <target>`.
...
- When the user approves "all" and every remaining diagnostic is fixable by the linter, you may run once: `python resources/cli-tools/frida_lint.py check --fix <target>` (and `--unsafe-fixes` only if the user explicitly allows it).
